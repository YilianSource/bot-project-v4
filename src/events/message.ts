...
import { Command, testArgument } from "../models/entities/command";
import { BotEvent } from "../models/entities/event";
import { ArgumentType } from "../models/enums/argumentType";
import { CommandType } from "../models/enums/commandType";
...
export default new BotEvent({
    name: "message",
    callback: async (bot: Client, msg: Message): Promise<unknown> => {
        ...
        const message = msg.content.substring(0);

        if (
            message.substring(0, guildState.prefix.length) == guildState.prefix
        ) {
            if (commands.has(args[0])) {
                ...
                try {
                    ...
                    if (
                        recentCommands.includes(`${msg.author.id}-${args[0]}`)
                    ) {
                        return msg.channel.send(
                    ...
                    if (
                        !command.args.every((argTypes, index) => {
                            ...
                            if (!Array.isArray(argTypes)) {
                                argTypes = [
                                    argTypes as unknown as ArgumentType,
                                ];
                            }
